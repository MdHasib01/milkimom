...
async function authRequest<T = any>(url: string, options?: RequestInit): Promise<ApiResult<T>> {
  try {
...
export async function adminLogin(email: string, password: string): Promise<ApiResult<{ token: string; user: AdminUser }>> {
...
export function fetchOrders(params?: { status?: string; phone?: string; page?: number; limit?: number }) {
